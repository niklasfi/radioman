...
namespace NextFunctor {
    ...
    template <> 
    constexpr long µs<Second>(){
        return 1e6;
    }
    
    template <> 
    constexpr long µs<Minute>(){
        return µs<Second>() * 60;
    }
    
    template <>
    constexpr long µs<Hour>(){
        return µs<Minute>() * 60;
    }
    
    template <typename T>
    ptime ceil(ptime t, bool force_carry){
        long rem = t.time_of_day().total_microseconds() % µs<T>();
        if(!force_carry && rem == 0)
            return t;
        else
            return t + microseconds(-rem) + microseconds(µs<T>());
    }
    ...
    ptime Hour::operator()(const ptime& from, bool force_carry){
        ...
        auto diff = µs<Hour>() * this->hour - t.time_of_day().total_microseconds();
        if(diff < 0) diff += µs<Hour>() * 24;
    ...
    ptime Minute::operator()(const ptime& from, bool force_carry){
        ...
        auto diff = µs<Minute>() * this->minute - t.time_of_day().total_microseconds() % µs<Hour>();
        if(diff < 0) diff += µs<Hour>();
        return t + microseconds(diff);
    }
    ...
    ptime Second::operator()(const ptime& from, bool force_carry){
        ...
        auto diff = µs<Second>() * this->second - t.time_of_day().total_microseconds() % µs<Minute>();
        if(diff < 0) diff += µs<Minute>();
        return t + microseconds(diff);
    }
